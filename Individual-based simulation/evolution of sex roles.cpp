/*========================================================================================================
Individual variation in parental care drives divergence of sex roles.cpp
==========================================================================================================
...
double  mp;					// mutational rate for gene p (preference)
double  signmap;			// sd step mutation size for p 
double  pMaxmutstep;		// maximum mutation stepsize possible for p 
double  mt;					// mutational rate for tau genes 
double  signmaTau;			// sd step mutation size for tau 
double  tauMaxmutstep;		// maximum mutation stepsize possible for tau
...
std::bernoulli_distribution sex(0.5);  // 50% is female and 50% is male
...
{ // the mated pairs switch to the caring state and produce offspring
	...
	if (surMale(rng))
	{  // if the male still survives after caring, move it to the caring state
		maleCareT = static_cast <double> (mateMale[maleID].getpc2());
		careMale.push_back(mateMale[maleID]);
	}
	else
	{ // if the male would die, expected care duration has been calculated
	...
	// male and female produce the offspring after mating
	double totalCare = maleCareT + femaleCareT + maleCareT * femaleCareT * syn;
	double surPro = (totalCare * totalCare) / (totalCare * totalCare + demandOff * demandOff);  // survival probabilty of offspring
	if (surPro > uniDis(rng)) // if the offspring can survive under the care
	{
	...
	case 0:
		...
		{
			// if the offspring could survive until be mature
	...
	case 1:
		...
		{
			// if the offspring survive after caring
			int newNumSurMaleOff = mateMale[maleID].getNumSurOff() + 1;
			int newNumSurFemeOff = mateFemale[femID].getNumSurOff() + 1;
			mateMale[maleID].writeNumSurOff(newNumSurMaleOff);
			mateFemale[femID].writeNumSurOff(newNumSurFemeOff);
...
{	// in the mating state, mating is either random or involved in sexual selection
	checkMatingDeath(t);

	if (mateMale.size() > 0 && mateFemale.size() > 0)
	{
		std::random_shuffle(mateFemale.begin(), mateFemale.end());
		if (ifRandom == 0) { // if mating is random
			for (int i = 0; i < mateFemale.size(); ++i)
			{
				if (mateMale.size() > 0)
				{
		...
		{	// if sexual selection is considered
			for (int i = 0; i < mateFemale.size(); ++i)
			{
				if (mateMale.size() > 0) {
					...
					{  // if the male is attractive to the female
						ifmate(i, m, t);
						--i;
					}
				}
...
// males and females would go to recovery pool after caring
...
{	// individuals enter the pre-mating state after caring 
	...
	for (int i = 0; i < careMale.size();)
	{
		...
		else
		{
			...
			std::bernoulli_distribution surMale(surProMale);
			if (surMale(rng))
			{  // if the male still survives after pre-mating
...
void juvToMating()
{	// juveniles enter the mating state after be mature
...
void readParamaters(const std::string &parFileName)
{
	...
	for (;;) {
		...
		if (ifs.good()) {
			...
			/// a switch() might be very useful here
			if (parID == "varp") {
				ifs >> varp;
...
{
	try {
		for (int file_number = 1; file_number <= 1; ++file_number) {
			...
			for (int simulation_number = 1; simulation_number <= 2; ++simulation_number)
			{
				...
				offspringTrait << "day" << ',' << "sex" << ',' << "malep1" << ',' << "malet" << ',' << "malepc1" << ',' << "malepc2" << ','
				...
				for (int day = 0; day < tEnd; ++day)
				{
					...
					// males and female mate randomly
					getCaringDays(day);

					// Individuals would leave caring pool once they finished the care
					gotoRecover(day);

					// Individuals would come back to the mating pool after recovery
					backtoMating(day);

					// ofsspring would go to mating pool after getting independent
					juvToMating();
